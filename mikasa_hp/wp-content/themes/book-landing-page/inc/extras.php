 ...
 */
function book_landing_page_load_preload_local_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not. Basically avoiding 'book_landing_page_WebFont_Loader' class rendering.
	$local_font_files = get_site_option( 'book_landing_page_local_font_files', false );

	if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
		$font_format = apply_filters( 'book_landing_page_local_google_fonts_format', $format );
	...
	$font = book_landing_page_webfont_loader_instance( $url );
	$font->set_font_format( $format );
	$font->preload_local_fonts();
}

if( ! function_exists( 'book_landing_page_flush_local_google_fonts' ) ){
	/**
	 * Ajax Callback for flushing the local font
	 */
  function book_landing_page_flush_local_google_fonts() {
    // Verify nonce for CSRF protection
    if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'book_landing_page_flush_fonts_nonce' ) ) {
    ...
    $WebFontLoader = new Book_Landing_Page_WebFont_Loader();
    //deleting the fonts folder using ajax
    $WebFontLoader->delete_fonts_folder();
    wp_send_json_success();
  }
}
add_action( 'wp_ajax_flush_local_google_fonts', 'book_landing_page_flush_local_google_fonts' );
