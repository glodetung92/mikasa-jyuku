...
function book_landing_page_scripts(){
    ...
    if( get_theme_mod( 'ed_localgoogle_fonts',false ) && ! is_customize_preview() && ! is_admin() ){
        if ( get_theme_mod( 'ed_preload_local_fonts',false ) ) {
			book_landing_page_load_preload_local_fonts( book_landing_page_get_webfont_url( book_landing_page_fonts_url() ) );
        }
        wp_enqueue_style( 'book-landing-page-google-fonts', book_landing_page_get_webfont_url( book_landing_page_fonts_url() ) );
    }else{
    ...
    wp_enqueue_script( 'book-landing-page-modal-accessibility', get_template_directory_uri() . '/js' . $build . '/modal-accessibility' . $suffix . '.js', array( 'jquery' ), BOOK_LANDING_PAGE_THEME_VERSION, true );
...
if( ! function_exists( 'book_landing_page_block_editor_styles' ) ) :
    /**
     * Enqueue editor styles for Gutenberg
     */
    function book_landing_page_block_editor_styles() {
    // Use minified libraries if SCRIPT_DEBUG is false
    $build  = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '/build' : '';
    $suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
    
    // Block styles.
    wp_enqueue_style( 'book-landing-page-block-editor-style', get_template_directory_uri() . '/css' . $build . '/editor-block' . $suffix . '.css' );

    // Add custom fonts.
    wp_enqueue_style( 'book-landing-page-google-fonts', book_landing_page_fonts_url(), array(), null );

}
endif;
add_action( 'enqueue_block_editor_assets', 'book_landing_page_block_editor_styles' );
...
function book_landing_page_admin_notice(){
    ...
    $dismissnonce   = wp_create_nonce( 'book_landing_page_admin_notice' );
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><a href="<?php echo esc_url( admin_url( 'themes.php?page=book-landing-page-dashboard' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Go to the dashboard.', 'book-landing-page' ); ?></a></p>
                    <p class="dismiss-link"><strong><a href="?book_landing_page_admin_notice=1&_wpnonce=<?php echo esc_attr( $dismissnonce ); ?>"><?php esc_html_e( 'Dismiss', 'book-landing-page' ); ?></a></strong></p>
                </div>
            </div>
        </div>
    <?php }
...
function book_landing_page_update_admin_notice(){
    ...
    if ( isset( $_GET['book_landing_page_admin_notice'] ) && $_GET['book_landing_page_admin_notice'] = '1' && wp_verify_nonce( $_GET['_wpnonce'], 'book_landing_page_admin_notice' )) {
